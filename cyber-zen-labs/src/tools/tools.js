...
const Tools = {
    ...
    getResponseCoords: (coords, size) => {
        ...
        const arrayCoordsXY = Tools.getCoordsAfterResponse(arrayCoefficient, size)
        const newArray = Tools.formatToArray(arrayCoordsXY, coords)
    ...
    getCoordsAfterResponse: (coefficientList, size) => {
        ...
        coefficientList.forEach((item, i) => {
            ...
            item.x.forEach((_item, _i) => {
                coords[coords.length - 1]?.x.push(_item === Infinity ? 0 : size.width / _item)


            })
            item.y.forEach((_item, _i) => {

                coords[coords.length - 1]?.y.push(_item === Infinity ? 0 : size.height / _item)
            })
        })
